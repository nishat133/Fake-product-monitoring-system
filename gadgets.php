<?php
	...
	$sql = "SELECT * FROM products where category=4 ORDER BY pid "; 
...
<div class="sidenav">
  ...
  <a href="men.php?login=<?php echo $login; ?>">Dress Collection</a>
   <a href="food.php?login=<?php echo $login; ?>">Food</a>
  <a href="women.php?login=<?php echo $login; ?>">Shoes</a>
...
<?php
	...
	while($row = mysqli_fetch_assoc($result))
	{
		...
		echo"<td><div class = 'box'><img src = 'images/gadgets/{$row['image']}' alt = '{$row['pid']}'>
		<h4><b>{$row['pname']}<b></h4>
		<h3>Price: <b>Rs.{$row['price']}</b></h3>
		<br>
		<form action = 'gadgets_shop.php?pid=$pid & login=$login & username=$username' method = 'post'>
